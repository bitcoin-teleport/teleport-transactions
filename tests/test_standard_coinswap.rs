...
async fn test_standard_coinswap() {
    ...
    assert_eq!(
        taker_wallet.list_unspent_from_wallet(&rpc).unwrap().len(),
        3
    );
    assert_eq!(
        maker1_wallet.list_unspent_from_wallet(&rpc).unwrap().len(),
        3
    );
    assert_eq!(
        maker2_wallet.list_unspent_from_wallet(&rpc).unwrap().len(),
        3
    );
